...
    // continúa a resolver por media id
  }
...
  throw new Error(
    `post_history row not found for ${
      platform
    } id=${platformMediaId} (and provided post_history_id "${idTrim}" didn't match).`
  );
}
...
async function fetchIGInsights(mediaId: string): Promise<Pick<Metrics, "impressions" | "reach" | "saves">> {
...
  if (!r.ok) {
    const txt = await r.text();
    console.warn(`IG insights soft-fail: ${r.status} ${txt}`);
    return {};
  }
  const j = await r.json();
...
  // INICIO: Código insertado
  const nothingUseful =
    (m.impressions ?? null) === null && // Ajustado (m.views no existe en el type Metrics)
    m.reach == null &&
    m.saves == null &&
    m.likes == null &&
    m.comments == null &&
    m.shares == null;
  if (nothingUseful) return m; // no insertes basura
